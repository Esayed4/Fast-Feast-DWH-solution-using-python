...
from transformer.generate_dim_date import generate_dim_date
...
def run_batch():
    ...
    if conn is None:
        logger.critical("Could not connect to DWH — aborting")
        return
    if pii_conn is None:
        logger.critical("Could not connect to PII DB — aborting")
        return
    ...
    for batch_folder in get_unprocessed_batches():
        ...
        try:
            ...
            for table_key in SCD2_TABLES:
                ...
                logger.info(f"[OK] {table_key} — {quality['records_clean']} rows processed")
            ...
            if max_date is None or batch_dt > max_date:
                ...
                logger.info(f"[OK] dim_date generated from {start_dt} to {end_dt}")
            else:
                logger.info(f"[SKIP] dim_date already covers {batch_date}")

            # Step 6: retry streaming orphans now that dimensions are updated
            logger.info("--- Retrying streaming orphans ---")
            try:
                handle_order_orphan()
                logger.info("[OK] Order orphans retried")
            except Exception as e:
                logger.warning(f"[WARN] Order orphan retry failed — {e}")

            try:
                handle_ticket_orphan()
                logger.info("[OK] Ticket orphans retried")
            except Exception as e:
                logger.warning(f"[WARN] Ticket orphan retry failed — {e}")

            # Step 7: mark batch as complete
            mark_batch_processed(batch_date)
            logger.info(f"[DONE] Batch {batch_date} completed successfully")

        except Exception as e:
            import traceback
            logger.error(f"[FAILED] Batch {batch_date} failed — {e}")
